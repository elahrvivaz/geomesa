...
//  def d2 = combine(z >> 2)
  ...
  def decode: (Int, Int) = (d0, d1)

  def dim(i: Int): Int = if (i == 0) d0 else if (i == 1) d1 else {
    throw new IllegalArgumentException(s"Invalid dimension $i - valid dimensions are 0,1")
  }

  def inRange(rmin: Z2, rmax: Z2): Boolean = {
    val (x, y) = decode
    x >= rmin.d0 && x <= rmax.d0 && y >= rmin.d1 && y <= rmax.d1
  }
  ...
  def bitsToString = f"(${z.toBinaryString.toLong}%016d)(${d0.toBinaryString.toLong}%08d,${d1.toBinaryString.toLong}%08d)"
...
object Z2 {
  ...
  def apply(zvalue: Long) = new Z2(zvalue)
