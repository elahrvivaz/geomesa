    ...
    val iter = new CloseableIterator[Array[Byte]] {
        ...
        loader.vector.getChildrenFromFields.map { child =>
          ...
          val transfer = if (child.getField.getDictionary == null) {
            ...
            (fromIndex: Int, toIndex: Int, _: collection.Map[Integer, Integer]) => {
              transfer.copyValueSafe(fromIndex, toIndex)
            }
          } else {
      ...
      override def next(): Array[Byte] = {
        ...
        while (total < batchSize && hasNext) {
          val remaining = math.min(batchSize - total, loader.vector.getAccessor.getValueCount - count)
  ...
  private def reduceWithSort(sft: SimpleFeatureType,
    ...
    threadedBatches.foreachIndex { case (batches, batchIndex) =>
      ...
      while (j < batches.length) {
        ...
        val map = loader.vector.asInstanceOf[NullableMapVector]
        val sort = ArrowAttributeReader(sft.getDescriptor(sortBy), map.getChild(sortBy), dictionaries.get(sortBy), encoding)
        mergeBuilder.+=((map, sort, transfers, mappings.get(sortBy).orNull))
        j += 1
      }
    }
    ...
    toMerge.foreachIndex { case ((vector, sort, _, mappings), i) =>
      if (vector.getAccessor.getValueCount > 0) {
        queue.+=((getSortAttribute(sort, mappings, 0), 0, i))
      }
    }
  ...
  private def mergeDictionaries(sft: SimpleFeatureType,
    ...
    val allMerges: Array[(Array[ArrowAttributeReader], Array[TransferPair], Array[HashBiMap[Integer, Integer]])] = deltas.map { deltas =>
      ...
      val offsets: Array[Array[Int]] = Array.tabulate(toMerge.length) { batch =>
        ...
        while (i < batch) {
          // set the count for each batch based so we can offset mappings later
