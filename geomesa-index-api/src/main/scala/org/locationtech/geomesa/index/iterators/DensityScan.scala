...
trait DensityScan extends AggregatingScan[DensityResult] {
  ...
  override protected def initResult(sft: SimpleFeatureType, options: Map[String, String]): DensityResult = {
...
object DensityScan {
  ...
  def configure(sft: SimpleFeatureType,
    ...
    val base = AggregatingScan.configure(sft, index, filter, hints.getTransform)
    val required = Map(
      EnvelopeOpt -> s"${envelope.getMinX},${envelope.getMaxX},${envelope.getMinY},${envelope.getMaxY}",
      GridOpt -> s"$width,$height"
    )
    val optional = weight.map(w => WeightOpt -> w).toMap
