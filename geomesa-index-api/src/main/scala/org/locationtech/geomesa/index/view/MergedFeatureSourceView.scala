...
class MergedFeatureSourceView(
  ...
  override def getCount(query: Query): Int = {
    val total =
      if (parallel) {
        ...
        sources.toList.map(s => CachedThreadPoolOps.submit(() => results.add(getSingle(s)))).foreach(_.get)
  ...
  override def getBounds: ReferencedEnvelope = {
    ...
    val sourceBounds = if (parallel) {
      ...
      sources.toList.map(s => CachedThreadPoolOps.submit(() => getSingle(s).foreach(results.add))).foreach(_.get)
      results.asScala
    } else {
