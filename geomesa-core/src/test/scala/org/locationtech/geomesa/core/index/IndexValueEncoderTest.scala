...
class IndexValueEncoderTest extends Specification {
  ...
  "IndexValueEncoder" should {
    ...
    "encode and decode id,geom,date" in {
      ...
      // inputs
      val entry = ScalaSimpleFeatureFactory.buildFeature(sft,
      ...
      decoded.getDefaultGeometry mustEqual(geom)
...
//      skipped("for integration")
      ...
      println(s"old $totalEncodeOld $totalDecodeOld - ${totalEncodeOld + totalDecodeOld}")
      println(s"new $totalEncodeNew $totalDecodeNew - ${totalEncodeNew + totalDecodeNew}")
      println
      success
    }
  }
