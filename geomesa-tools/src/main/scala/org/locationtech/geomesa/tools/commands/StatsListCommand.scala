...
class StatsListCommand(parent: JCommander) extends CommandWithCatalog(parent) with LazyLogging {
  ...
  private def getExactStat(sft: SimpleFeatureType,
                           ...
                           filter: Filter,
                           bins: Option[Int]): Option[RangeHistogram[Any]] = {
    import GeoMesaMetadataStats.{DefaultHistogramSize, GeometryHistogramSize}
    import org.locationtech.geomesa.utils.geotools.RichSimpleFeatureType.RichSimpleFeatureType

    System.err.println(s"Calculating stats for '$attribute'__.")

    def getEndpoints(exact: Boolean) = ds.stats.getMinMax[Any](sft, attribute, filter, exact = exact)

    getEndpoints(false).orElse(getEndpoints(true)).map { bounds =>
      val length = bins.getOrElse(if (sft.getGeomField == attribute) GeometryHistogramSize else DefaultHistogramSize)
...
object StatsListCommand {
  ...
  private val Threshold1 = (Console.CYAN + Console.BOLD, '\u26AB') // black circle
  private val Threshold2 = (Console.GREEN + Console.BOLD, '\u25CF') // medium black circle
  private val Threshold3 = (Console.YELLOW + Console.BOLD, '\u25CF') // medium black circlt
  private val Threshold4 = (Console.RED + Console.BOLD, '\u2B24') // black circle large
