...
class ShpIngestTest extends Specification {
  ...
  "ShpIngest" >> {
    ...
    "should properly ingest a shapefile" >> {
      ...
      ds.stats.getMinMax[Date](ds.getSchema("shpingest"), "dtg").bounds must beSome((minDate, maxDate))
    ...
    "should support renaming the feature type" >> {
      ...
      ds.stats.getMinMax[Date](ds.getSchema("changed"), "dtg").bounds must beSome((minDate, maxDate))
