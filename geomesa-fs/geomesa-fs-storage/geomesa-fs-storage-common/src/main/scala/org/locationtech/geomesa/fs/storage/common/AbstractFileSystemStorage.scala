...
abstract class AbstractFileSystemStorage(
  ...
  override def compact(partition: Option[String], fileSize: Option[Long], threads: Int): Unit = {
    partition.map(Seq(_)).getOrElse(metadata.getPartitions().map(_.name)).foreach { partition =>
      val toCompact = getFilePaths(partition).filter { path =>
        if (!fileIsSized(path.path, fileSize)) { true } else {
          logger.debug(s"Skipping compaction for file [$path] (already target size)")
          false
        }
      }
      ...
      if (toCompact.lengthCompare(2) < 0) {
        ...
        def writer: FileSystemWriter =
          createWriter(partition, StorageFileAction.Append, FileType.Compacted, fileSize)
  ...
  private def createWriter(
    ...
    estimator(targetFileSize) match {
      case None => createWriter(pathAndObserver)
      case Some(e) => new ChunkedFileSystemWriter(Iterator.continually(pathAndObserver), e)
    }
  }
