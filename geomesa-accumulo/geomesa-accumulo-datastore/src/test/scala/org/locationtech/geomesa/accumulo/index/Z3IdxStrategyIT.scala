...
class Z3IdxStrategyIT extends TestWithFeatureType {

  sequential // note: test doesn't need to be sequential but it actually runs faster this way

  override val spec = "name:String,track:String,dtg:Date,*geom:Point:srid=4326;geomesa.indexes.enabled='z3'"
  ...
  def runSerialQuery(query: Query): Iterator[SimpleFeature] =
  ...
  "Z3IdxStrategy" should {
    ...
    "return all features for inclusive filter" >> {
      ...
      val features = runQuery(new Query(sftName, ECQL.toFilter(filter)))
    ...
    "apply transforms" >> {
      val filter = "bbox(geom, 38, 59, 51, 61)" +
          " AND dtg between '2010-05-07T06:00:00.000Z' and '2010-05-08T00:00:00.000Z'"
      val features = runQuery(new Query(sftName, ECQL.toFilter(filter), Array("name")))
      features must haveSize(4)
      features.map(_.getID.toInt) must containTheSameElementsAs(6 to 9)
      forall(features) { f =>
    ...
    "apply functional transforms" >> {
      val filter = "bbox(geom, 38, 59, 51, 61)" +
          " AND dtg between '2010-05-07T06:00:00.000Z' and '2010-05-08T00:00:00.000Z'"
      val features = runQuery(new Query(sftName, ECQL.toFilter(filter), Array("derived=strConcat('my', name)")))
      features must haveSize(4)
      features.map(_.getID.toInt) must containTheSameElementsAs(6 to 9)
      forall(features) { f =>
        f.getAttributeCount mustEqual 1
    ...
    "optimize for bin format" >> {
      ...
      val aggregates = try {
        ...
        runSerialQuery(query).map(f => f.getAttribute(BIN_ATTRIBUTE_INDEX).asInstanceOf[Array[Byte]]).toSeq
      } finally {
    ...
    "support sampling" >> {
      ...
      QueryProperties.ScanRangesTarget.threadLocalValue.set("1")
      val results = try { runQuery(query) } finally {
        QueryProperties.ScanRangesTarget.threadLocalValue.remove()
      }
    ...
    "support sampling with bin queries" >> {
      ...
      val results = try {
        ...
        runSerialQuery(query).map(_.getAttribute(BIN_ATTRIBUTE_INDEX)).toList
      } finally {
        QueryProperties.ScanRangesTarget.threadLocalValue.remove()
      }
