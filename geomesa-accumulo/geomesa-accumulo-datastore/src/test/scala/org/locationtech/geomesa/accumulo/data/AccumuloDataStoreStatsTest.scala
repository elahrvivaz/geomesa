  ...
  "AccumuloDataStore" should {
    "track stats for ingested features" >> {
      ...
      "initially have global stats" >> {
        ds.stats.getCount(sft) mustEqual -1
        ds.stats.getBounds(sft) mustEqual wholeWorldEnvelope
        ds.stats.getMinMax[String](sft, "name").bounds must beNone
        ds.stats.getMinMax[Date](sft, "dtg").bounds must beNone
      }
      ...
      "through feature writer append" >> {
        ...
        ds.stats.getMinMax[String](sft, "name").bounds must beSome("alpha", "alpha")
        ds.stats.getMinMax[Date](sft, "dtg").bounds must beSome(new Date(baseMillis), new Date(baseMillis))
        ...
        ds.stats.getCount(sft) mustEqual 2
        ds.stats.getBounds(sft) mustEqual new ReferencedEnvelope(0, 10, 0, 10, CRS_EPSG_4326)
        ds.stats.getMinMax[String](sft, "name").bounds must beSome("alpha", "cappa")
        ds.stats.getMinMax[Date](sft, "dtg").bounds must
      ...
      "through feature source add features" >> {
        ...
        ds.stats.getCount(sft) mustEqual 3
        ds.stats.getBounds(sft) mustEqual new ReferencedEnvelope(-10, 10, -10, 10, CRS_EPSG_4326)
        ds.stats.getMinMax[String](sft, "name").bounds must beSome ("alpha", "gamma")
        ds.stats.getMinMax[Date](sft, "dtg").bounds must
      ...
      "not expand bounds when not necessary" >> {
        ...
        ds.stats.getCount(sft) mustEqual 4
        ds.stats.getBounds(sft) mustEqual new ReferencedEnvelope(-10, 10, -10, 10, CRS_EPSG_4326)
        ds.stats.getMinMax[String](sft, "name").bounds must beSome("alpha", "gamma")
        ds.stats.getMinMax[Date](sft, "dtg").bounds must
            beSome(new Date(baseMillis), new Date(baseMillis + dayInMillis))
      ...
      "update all stats" >> {
        ...
        val epochWeekStart = Z3Table.EPOCH.plusWeeks {
          Weeks.weeksBetween(Z3Table.EPOCH, new DateTime(minDate, DateTimeZone.UTC)).getWeeks
        }
        val epochWeekEnd = Z3Table.EPOCH.plusWeeks {
          Weeks.weeksBetween(Z3Table.EPOCH, new DateTime(maxDate, DateTimeZone.UTC)).getWeeks + 1
        }
        ...
        ds.stats.getCount(sft) mustEqual 11
        ds.stats.getBounds(sft) mustEqual new ReferencedEnvelope(0, 27, 0, 9, CRS_EPSG_4326)
        ds.stats.getMinMax[String](sft, "name").bounds must beSome("0", "9")
        ds.stats.getMinMax[Date](sft, "dtg").bounds must beSome(minDate, maxDate)
        ds.stats.getHistogram[String](sft, "name") match {
          case None => failure("name histogram should be some")
          case Some(hist) =>
            hist.endpoints mustEqual ("0", "9")
            hist.length mustEqual 100
        ...
        ds.stats.getHistogram[Date](sft, "dtg") match {
          ...
          case Some(hist) =>
            // the date histogram is broken out into epoch weeks to align with the z3 index
            hist.endpoints mustEqual (epochWeekStart.toDate, epochWeekEnd.toDate)
            hist.length mustEqual 2
            hist.count(0) mustEqual 7
            hist.count(1) mustEqual 4
        }
        success
      }
      ...
      "estimate counts for spatial queries" >> {
        ...
        forall(filters.map(ECQL.toFilter)) { filter =>
          ...
          calculated mustEqual exact
          estimated must beCloseTo(exact, 6L)
        }
      }
      ...
      "estimate counts for spatio-temporal queries" >> {
        ...
        forall(filters.map(ECQL.toFilter)) { filter =>
          ...
          val calculated = ds.stats.getCount(sft, filter, exact = true)
          calculated mustEqual exact
          estimated must beCloseTo(exact, 4L)
        }
      }
      ...
      "estimate counts for attribute queries" >> {
        ...
        forall(filters.map(ECQL.toFilter)) { filter =>
          ...
          val calculated = ds.stats.getCount(sft, filter, exact = true)
          calculated mustEqual exact
          estimated must beCloseTo(exact, 1L)
        }
      }
    }
  }
