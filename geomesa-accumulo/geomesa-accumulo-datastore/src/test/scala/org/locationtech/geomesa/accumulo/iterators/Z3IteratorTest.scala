...
import org.locationtech.geomesa.accumulo.index.z3.Z3IndexWritable
...
class Z3IteratorTest extends Specification {
  ...
  "Z3Iterator" should {
    ...
    "iterate on non-points" >> {
      val (xmin, ymin, tmin) = Z3(sfc.index(lx, ly, lt).z & Z3IndexWritable.GEOM_Z_MASK).decode
      val (xmax, ymax, tmax) = Z3(sfc.index(ux, uy, ut).z & Z3IndexWritable.GEOM_Z_MASK).decode
      ...
      "keep in bounds values" >> {
        val test1 = sfc.index(-76.0, 38.5, 500)
        val prefix = Array[Byte](0, 0)
        val row = Bytes.concat(prefix, Longs.toByteArray(test1.z).take(Z3IndexWritable.GEOM_Z_NUM_BYTES))
        srcIter.key = new Key(new Text(row))
        iter.next()
        iter.hasTop must beTrue
      }
      ...
      "drop out of bounds values" >> {
        val test2 = sfc.index(-70.0, 38.5, 500)
        val prefix = Array[Byte](0, 0)
        val row = Bytes.concat(prefix, Longs.toByteArray(test2.z).take(Z3IndexWritable.GEOM_Z_NUM_BYTES))
        srcIter.key = new Key(new Text(row))
        iter.next()
        iter.hasTop must beFalse
      }
