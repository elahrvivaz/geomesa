  ...
  "Z2IdxStrategy" should {
    "print values" in {
      ...
      ds.connector.createScanner(ds.getTableName(sftName, Z2Table), new Authorizations()).foreach { r =>
        ...
        val prefixLength = sft.getTableSharingPrefix.length
        val zlo = bytes.slice(prefixLength + 1, prefixLength + 5)
        val zhi = bytes.slice(prefixLength + 11, prefixLength + 15)
        val keyZ = Longs.fromByteArray(zlo ++ zhi)
...
//      forall(qps)(p => p.columnFamilies must containTheSameElementsAs(Seq(Z2Table.BIN_CF)))
//
...
//    }.pendingUntilFixed("not implemented")
    ...
    "optimize for bin format" >> {
      ...
      val filter = "bbox(geom, 35, 55, 45, 90)" +
