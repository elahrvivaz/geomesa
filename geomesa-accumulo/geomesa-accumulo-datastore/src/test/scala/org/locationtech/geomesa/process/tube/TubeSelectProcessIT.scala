...
class TubeSelectProcessIT extends TestWithFeatureTypes {

  import TubeBuilder.DefaultDtgField

  sequential

  lazy val tubeTestOne = createNewSchema(s"type:String,*geom:Point:srid=4326,$DefaultDtgField:Date")
  lazy val tubeTestTwo = createNewSchema(s"type:String,*geom:Point:srid=4326,$DefaultDtgField:Date")
  lazy val tubeTestLines = createNewSchema(s"type:String,*geom:Geometry:srid=4326,$DefaultDtgField:Date;geomesa.mixed.geometries=true")
  ...
  "TubeSelect" should {
    ...
    "should do a simple tube with geo + time interpolation" in {
      ...
      addFeatures(
        List("c").flatMap { name =>
          List(1, 2, 3, 4).zip(List(45, 46, 47, 48)).map { case (i, lat) =>
            ScalaSimpleFeature.create(sft, s"$name$i", name, f"POINT($lat%d $lat%d)", "2011-01-02T00:00:00Z")
          }
        }
      )
      ...
      addFeatures(
        List("a", "b").flatMap { name =>
          for (lon <- 40 until 50; lat <- 40 until 50) yield {
            val id = s"$name$i"
            i += 1
            ScalaSimpleFeature.create(sft, id, name, f"POINT($lon%d $lat%d)", "2011-01-02T00:00:00Z")
          }
        }
      )
    ...
    "should properly dedupe overlapping results based on buffer size " in {
      val sft = tubeTestTwo
    ...
    "should properly handle geometries crossing the IDL" in {
      ...
      addFeatures(
        List("c", "d").flatMap { name =>
          List(1, 2, 3, 4).zip(List(-178, -179, 179, 178)).map { case (i, lat) =>
            ScalaSimpleFeature.create(sft, s"$name$i", name, f"POINT($lat%d 0)", f"2011-01-01T00:0$i%d:00Z")
          }
        }
      )
      ...
      addFeature(ScalaSimpleFeature.create(sft, "d5", "d", f"POINT(-178.5 0)", f"2011-01-01T00:02:03Z"))
  ...
  "TubeSelect" should {
    "should handle all geometries" in {
      val sft = tubeTestLines

      addFeatures(
        List("b").flatMap{ name =>
          List(1, 2, 3, 4).zip(List(45, 46, 47, 48)).map { case (i, lat) =>
            ScalaSimpleFeature.create(sft, s"$name$i", name, f"POINT(40 $lat%d)", "2011-01-01T00:00:00Z")
          }
        }
      )

      addFeature(ScalaSimpleFeature.create(sft, "b-line", "b", "LINESTRING(40 40, 40 50)", "2011-01-01T00:00:00Z"))
      addFeature(ScalaSimpleFeature.create(sft, "b-poly", "b", "POLYGON((40 40, 41 40, 41 41, 40 41, 40 40))", "2011-01-01T00:00:00Z"))

      // tube features
      val tubeFeatures = new ListFeatureCollection(sft, Collections.singletonList[SimpleFeature](
        ScalaSimpleFeature.create(sft, "a-line", "a", "LINESTRING(40 40, 40 50)", "2011-01-01T00:00:00Z")
      ))
  ...
  "TubeBuilder" should {
    "approximate meters to degrees" in {
      ...
      forall(List(0, 30, 60, 89).zip(List(110.57, 110.85, 111.41, 111.69))) { case(lat, dist) =>
        val deg = new NoGapFill(new DefaultFeatureCollection("", tubeTestOne), 0, 0).metersToDegrees(110.57*1000, geoFac.createPoint(new Coordinate(0, lat)))
  ...
  "TubeSelect" should {
    "properly handle values for execute" in {
      ...
      val q = new Query(sft.getTypeName, Filter.INCLUDE)
      val res = ds.getFeatureSource(sft.getTypeName).getFeatures(q)
      ...
      new TubeSelectProcess().execute(tubeFeatures, res, null, null, null, null, null, null) must not(throwAn[ClassCastException])
    }
  }
  ...
  "TubeSelect" should {
    "should do a simple tube with geo interpolation with a different date field" in {
      ...
      val sft = createNewSchema(s"type:String,*geom:Point:srid=4326,$nonDefaultDtgField:Date")
