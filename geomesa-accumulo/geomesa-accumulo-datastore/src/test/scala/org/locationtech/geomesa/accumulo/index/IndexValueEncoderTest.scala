...
class IndexValueEncoderTest extends Specification {
  ...
  "IndexValueEncoder" should {
    "default to id,geom,date" in {
      val sft = getSft()
      IndexValueEncoder(sft, CURRENT_SCHEMA_VERSION).fields must containAllOf(Seq("geom", "dtg"))
    }
    "default to id,geom if no date" in {
      val sft = getSft("*geom:Geometry,foo:String")
      IndexValueEncoder(sft, CURRENT_SCHEMA_VERSION).fields must containAllOf(Seq("geom"))
    }
    "allow custom fields to be set" in {
      ...
      IndexValueEncoder(sft, CURRENT_SCHEMA_VERSION).fields must containAllOf(Seq("geom", "dtg", "i", "f"))
    }
    ...
    "encode and decode id,geom,date" in {
      ...
      val encoder = IndexValueEncoder(sft, CURRENT_SCHEMA_VERSION)
