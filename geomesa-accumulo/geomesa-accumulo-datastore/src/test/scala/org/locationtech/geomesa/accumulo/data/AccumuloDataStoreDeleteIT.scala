...
class AccumuloDataStoreDeleteIT extends TestWithFeatureTypes {

  sequential

  lazy val tableOps = ds.connector.tableOperations()

  def createFeature(schema: String = "name:String:index=join,*geom:Point:srid=4326,dtg:Date"): SimpleFeatureType = {
    val sft = createNewSchema(schema)
    addFeatures(Seq(ScalaSimpleFeature.create(sft, "fid-1", "1", "POINT(45.0 45.0)", "2012-01-02T05:06:07.000Z")))
    sft
  }
  ...
  "AccumuloDataStore" should {
    ...
    "delete all associated tables" >> {
      ...
      WithAccumuloStore(dsParams ++ Map(AccumuloDataStoreParams.CatalogParam.key -> catalog)) { ds =>
