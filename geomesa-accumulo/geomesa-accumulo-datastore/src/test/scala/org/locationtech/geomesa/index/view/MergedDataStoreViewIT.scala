...
import org.locationtech.geomesa.index.view.MergedDataStoreViewIT.TestConfigLoader
...
class MergedDataStoreViewIT extends TestWithMiniCluster {
  ...
  lazy val accumuloParams = Map(
    AccumuloDataStoreParams.InstanceIdParam.key -> instanceId,
    AccumuloDataStoreParams.ZookeepersParam.key -> zookeepers,
    AccumuloDataStoreParams.UserParam.key       -> connector.whoami(),
  ...
  "MergedDataStoreView" should {
    ...
    "load via SPI config" in {
      ...
      MergedDataStoreViewIT.loadConfig =
