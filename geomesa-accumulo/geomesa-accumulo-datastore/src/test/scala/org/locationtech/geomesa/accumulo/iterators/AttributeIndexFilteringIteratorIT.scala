...
class AttributeIndexFilteringIteratorIT extends TestWithFeatureType {
  ...
  "AttributeIndexFilteringIterator" should {
    ...
    "handle like queries and choose correct strategies" in {
      ...
      runQuery(wildCardQuery) must haveLength(16)
      ...
      forall(List("a", "b", "c", "d")) { letter =>
        ...
        runQuery(leftWildCard) must haveLength(4)
        ...
        runQuery(doubleWildCard) must haveLength(4)
        ...
        runQuery(rightWildcard) must haveLength(4)
      }
    }
