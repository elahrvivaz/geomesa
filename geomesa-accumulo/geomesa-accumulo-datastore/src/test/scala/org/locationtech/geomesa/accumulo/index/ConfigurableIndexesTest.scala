...
class ConfigurableIndexesTest extends Specification with TestWithDataStore {
  ...
  "AccumuloDataStore" should {
    "only create the z3 index" >> {
      ...
      forall(AccumuloFeatureIndex.AllIndices.filter(i => i != Z3Index && i != GeoHashIndex)) { i =>
        ds.tableOps.exists(ds.getTableName(sft.getTypeName, i)) must beFalse
      }
    }
    ...
    "add another empty index" >> {
      ...
      sft.setIndexVersion(Z2Index.name, Z2Index.version)
      ds.updateSchema(sftName, sft)
      forall(Seq(Z3Index, Z2Index))(i => ds.tableOps.exists(ds.getTableName(sft.getTypeName, i)) must beTrue)
      forall(AccumuloFeatureIndex.AllIndices.filter(i => i != Z3Index && i != Z2Index && i != GeoHashIndex)) { i =>
        ds.tableOps.exists(ds.getTableName(sft.getTypeName, i)) must beFalse
      }
