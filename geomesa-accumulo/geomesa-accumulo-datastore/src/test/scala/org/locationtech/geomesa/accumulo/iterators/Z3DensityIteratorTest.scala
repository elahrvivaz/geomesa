...
class Z3DensityIteratorTest extends Specification with TestWithDataStore {
  ...
  "Z3DensityIterator" should {
    ...
    "reduce total features returned" in {
      ...
      fs.getFeatures(q).features.length must beLessThan(150)
    }
    ...
    "maintain total weight of points" in {
      ...
      val q = getQuery("(dtg between '2012-01-01T18:00:00.000Z' AND '2012-01-01T23:00:00.000Z') and BBOX(geom, -80, 33, -70, 40)")
      val weights = fs.getFeatures(q).features().map(_.getAttribute("weight").asInstanceOf[Long])
      weights.sum mustEqual 150
    }
    ...
    "correctly bin points" in {
      ...
      val ptsAndWeights = fs.getFeatures(q).features().map { f =>
        (f.getAttribute("geom").asInstanceOf[Point], f.getAttribute("weight").asInstanceOf[Long])
      }.toSeq

      ptsAndWeights.map(_._2).sum mustEqual 150

      val compiled = ptsAndWeights.groupBy(_._1).map { case (pt, group) => group.map(_._2).sum }
