...
class VisibilitiesIT extends TestWithFeatureType {
  ...
  lazy val privDS = {
    val name = s"${getClass.getSimpleName}_priv"
    connector.securityOperations().createLocalUser(name, new PasswordToken("pass"))
    connector.securityOperations().changeUserAuthorizations(name, new Authorizations("user", "admin"))
    val cn = createConnector(name, "pass")
    DataStoreFinder.getDataStore(dsParams ++ Map(AccumuloDataStoreParams.ConnectorParam.key -> cn))
  }
  lazy val unprivDS = {
    val name = s"${getClass.getSimpleName}_unpriv"
    connector.securityOperations().createLocalUser(name, new PasswordToken("pass"))
    connector.securityOperations().changeUserAuthorizations(name, new Authorizations("user"))
    val cn = createConnector(name, "pass")
    DataStoreFinder.getDataStore(dsParams ++ Map(AccumuloDataStoreParams.ConnectorParam.key -> cn))
  }
