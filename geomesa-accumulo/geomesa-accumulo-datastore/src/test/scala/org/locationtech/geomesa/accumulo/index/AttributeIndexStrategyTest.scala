...
class AttributeIndexStrategyTest extends Specification with TestWithDataStore {
  ...
  "AttributeIdxStrategy merging" should {
    ...
    "merge PropertyIsEqualTo primary filters" >> {
      ...
      val res = AttributeQueryableIndex.tryMergeAttrStrategy(qf1, qf2)
      res must not(beNull)
    ...
    "merge PropertyIsEqualTo on multiple ORs" >> {
      ...
      val res = AttributeQueryableIndex.tryMergeAttrStrategy(AttributeQueryableIndex.tryMergeAttrStrategy(qf1, qf2), qf3)
      res must not(beNull)
