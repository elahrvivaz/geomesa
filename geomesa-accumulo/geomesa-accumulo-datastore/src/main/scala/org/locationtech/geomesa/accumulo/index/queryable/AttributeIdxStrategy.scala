...
import java.util.{Collection, Date, Locale}
...
import org.apache.accumulo.core.data.{Key, Mutation, Value, Range => AccRange}
import org.apache.hadoop.io.Text
import org.calrissian.mango.types.TypeEncoder
import org.geotools.data.DataUtilities
import org.geotools.factory.Hints
import org.locationtech.geomesa.accumulo.data.tables.AttributeTable._
import org.locationtech.geomesa.accumulo.data.{WritableFeature, _}
import org.locationtech.geomesa.accumulo.data.tables.{AttributeTable, RecordTable}
import org.locationtech.geomesa.accumulo.index.AccumuloIndexStrategy
import org.locationtech.geomesa.accumulo.index.QueryHints.RichHints
import org.locationtech.geomesa.accumulo.index.QueryPlanners.{FeatureFunction, JoinFunction}
import org.locationtech.geomesa.accumulo.iterators._
import org.locationtech.geomesa.api.index.{QueryFilter, QueryPlan, QueryableIndex, WritableFeature}
import org.locationtech.geomesa.api.stats.GeoMesaStats
import org.locationtech.geomesa.api.utils.ExplainQuery
...
object AttributeQueryable extends QueryableIndex[java.util.Map.Entry[Key, Value]] with LazyLogging {
...
//  }
  ...
  override def getQueryPlan(sft: SimpleFeatureType,
                            filter: QueryFilter[Entry[Key, Value]],
                            hints: Hints,
                            explain: ExplainQuery): QueryPlan[Entry[Key, Value]] = {

  }

  override def getSimpleQueryFilter(filter: Filter): Option[QueryFilter[Entry[Key, Value]]] = {
...
object AttributeIdxStrategy extends StrategyProvider {
  ...
  def tryMergeAttrStrategy(toMerge: QueryFilter, mergeTo: QueryFilter): QueryFilter = {
    ...
    if (canMergePrimary && toMerge.secondary == mergeTo.secondary) {
      QueryFilter(mergeTo.index, orOption(toMerge.primary.toSeq ++ mergeTo.primary), mergeTo.secondary)
    } else {
      null
    }
  }
