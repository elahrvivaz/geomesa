...
case class QueryPlanner(sft: SimpleFeatureType, ds: AccumuloDataStore) extends MethodProfiling {
  ...
  private def getQueryPlans(query: Query,
    ...
    val filterPlan =
      AccumuloStrategyDecider.chooseFilterPlan(sft, stats, q.getFilter, transform, requestedIndex, explain)
    explain.popLevel()
    ...
    filterPlan.strategies.iterator.flatMap { strategy =>
      ...
      val plan = profile(strategy.index.getQueryPlan(sft, ds, strategy, hints, explain))
