...
object StatsCombiner {
  val SftOption = "sft-"
  val SftOptionLength = SftOption.length
}
...
class StatsCombiner extends Combiner with LazyLogging {
  ...
  import StatsCombiner.{SftOption, SftOptionLength}
  ...
  override def init(source: SortedKeyValueIterator[Key, Value],
    ...
    serializers = options.toMap.collect {
      case (k, v) if k.startsWith(SftOption) =>
        val typeName = k.substring(SftOptionLength)
  ...
  override def reduce(key: Key, iter: java.util.Iterator[Value]): Value = {
    ...
    if (!iter.hasNext) {
      ...
      val sftName = AccumuloBackedMetadata.getFeatureNameFromMetadataRowKey(key.getRow.toString)
