...
class DensityIterator extends KryoLazyDensityIterator with Logging {
  ...
  override def writeMultiPoint(geom: MultiPoint, weight: Double, result: DensityResult): Unit = {
  ...
  override def writeLineString(geom: LineString, weight: Double, result: DensityResult): Unit = {
    val geohash = indexDecoder.decode(source.getTopKey).getDefaultGeometry.asInstanceOf[Geometry]
  ...
  override def writePolygon(geom: Polygon, weight: Double, result: DensityResult): Unit = {
    val geohash = indexDecoder.decode(source.getTopKey).getDefaultGeometry.asInstanceOf[Geometry]
