...
object KryoLazyMapAggregatingIterator extends Logging {
  ...
  def reduceMapAggregationFeatures(features: SFIter, query: Query): SFIter = {
    ...
    val mapIndex = sft.indexOf(query.getHints.get(MAP_AGGREGATION_KEY).asInstanceOf[String])
    ...
    if (maps.nonEmpty) {
      ...
      val attributes = Array(reducedMap, GeometryUtils.zeroPoint) // filler value as feature requires a geometry
