  ...
  def validateDtgField(sft: SimpleFeatureType): Unit = {
    ...
    if (!sft.getDtgField.exists(dtgCandidates.contains)) {
      ...
      dtgCandidates.headOption.foreach { candidate =>
        val dtgProp = org.locationtech.geomesa.utils.geotools.RichSimpleFeatureType.DEFAULT_DATE_KEY
        lazy val theWarning =
          s"""
             |$dtgProp is not valid or defined for simple feature type $sft.
             |However, the following attribute(s) can be used in GeoMesa's temporal index:
             |${dtgCandidates.mkString("\n", ", ", "\n")}
             |GeoMesa will now point $dtgProp to the first temporal attribute found, so that the index will
             |include a temporal component: $candidate
        """.stripMargin
