...
import org.locationtech.geomesa.accumulo.index.attribute.AttributeWritableIndexV5
...
class AttributeIndexIterator
  ...
  def setAttributeFromRow(key: Key, sf: SimpleFeature) = {
    ...
    val decoded = AttributeWritableIndexV5.decodeAttributeIndexRow(attributeRowPrefix, attributeType, row)
    decoded match {
