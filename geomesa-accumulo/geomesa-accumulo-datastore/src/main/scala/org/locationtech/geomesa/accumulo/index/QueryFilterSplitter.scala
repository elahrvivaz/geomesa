...
class QueryFilterSplitter(sft: SimpleFeatureType) extends LazyLogging {
  ...
  def getQueryOptions(filter: Filter): Seq[FilterPlan] = {
    ...
    rewriteFilterInCNF(filter) match {
      case a: And =>
        if (!a.getChildren.exists(c => c.isInstanceOf[Or] && attributeAndIdCount(c) > 1)) {
      ...
      case o: Or =>
        ...
        if (attributeAndIdCount(o) == 1) {
          // just a simple or between a single field
          getSimpleQueryOptions(o).map(FilterPlan.apply)
        } else {
          ...
          val perAttributeOptions =
            o.getChildren.groupBy(getGroup).values.map(ff.or(_)).map(getSimpleQueryOptions).toSeq
          if (perAttributeOptions.exists(opt => opt.length == 1 && opt.head.primary.isEmpty)) {
            // we have to do a full table scan for part of the query, just append everything to that
            Seq(FilterPlan(fullTableScanOption(o)))
          } else {
            // get the permutations for the different options
            // note: this is O(M^N), where M is the number of options for each filter (should be 1 or 2)
            // and N is the number of discrete attributes being queried
            val options = perAttributeOptions.foldLeft(Seq.empty[Seq[QueryFilter]]) {
              (result, options) => if (result.isEmpty) Seq(options) else result.flatMap(r => options.map(r ++ Seq(_)))
            }
            options.map(FilterPlan.apply).map(makeDisjoint)
          }
        }
  ...
  private def isBounded(temporalFilter: Filter): Boolean = {
    ...
    intervals.exists { case (start, end) => start != MinDateTime && end != MaxDateTime }
  }
}
...
case class QueryFilter(strategy: StrategyType, primary: Option[Filter], secondary: Option[Filter] = None) {
  ...
  override lazy val toString: String =
    s"$strategy[${primary.map(filterToString).getOrElse("INCLUDE")}][${secondary.map(filterToString).getOrElse("None")}]"
}
