...
case class QueryStat(typeName: String,
                     ...
                     scanTime: Long,
                     hits:     Long,
                     deleted:  Boolean = false) extends Stat
...
object QueryStatTransform extends StatTransform[QueryStat] {
  ...
  private [stats] val CQ_USER     = new Text("user")
  private [stats] val CQ_FILTER   = new Text("queryFilter")
  private [stats] val CQ_HINTS    = new Text("queryHints")
  private [stats] val CQ_PLANTIME = new Text("timePlanning")
  private [stats] val CQ_SCANTIME = new Text("timeScanning")
  private [stats] val CQ_TIME     = new Text("timeTotal")
  private [stats] val CQ_HITS     = new Text("hits")
  private [stats] val CQ_DELETED  = new Text("deleted")
...
case class SerializedQueryStat(typeName: String,
                               date:     Long,
                               deleted:  Boolean,
                               entries:  Map[(Text, Text), Value]) extends Stat {
  lazy val user = entries.find(_._1._2 == QueryStatTransform.CQ_USER).map(_._2.toString).getOrElse("unknown")
}

object SerializedQueryStatTransform extends StatTransform[SerializedQueryStat] {
