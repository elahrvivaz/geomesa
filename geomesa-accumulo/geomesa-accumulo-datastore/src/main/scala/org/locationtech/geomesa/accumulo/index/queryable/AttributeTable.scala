...
package org.locationtech.geomesa.accumulo.index.queryable
...
import org.locationtech.geomesa.accumulo.index.{AccumuloMutableIndex, RowValue}
import org.locationtech.geomesa.api.index.WritableFeature
...
//  override val name: String = "attr"
  ...
  override def writer(sft: SimpleFeatureType): FeatureToMutation = {
    val getRows = getRowKeys(sft)
    if (sft.getSchemaVersion < 9) {
      (wf: WritableFeature[RowValue]) => {
  ...
  override def remover(sft: SimpleFeatureType): FeatureToMutation = {
    val getRows = getRowKeys(sft)
    if (sft.getSchemaVersion < 9) {
      (wf: WritableFeature[RowValue]) => {
        getRows(wf).map { case (descriptor, row) =>
  ...
  private def getRowKeys(sft: SimpleFeatureType): (WritableFeature[RowValue]) => Seq[(AttributeDescriptor, Array[Byte])] = {
    ...
    val getSuffix: (WritableFeature[RowValue]) => Array[Byte] = sft.getDtgIndex match {
      case None => (fw: WritableFeature[RowValue]) => fw.feature.getID.getBytes(StandardCharsets.UTF_8)
      case Some(dtgIndex) =>
        (fw: WritableFeature[RowValue]) => {
  ...
  private def getRowKeys(indexedAttributes: Seq[(AttributeDescriptor, Int, Array[Byte])],
                         prefix: Array[Byte],
                         suffix: (WritableFeature[RowValue]) => Array[Byte])
                        (fw: WritableFeature[RowValue]): Seq[(AttributeDescriptor, Array[Byte])] = {
