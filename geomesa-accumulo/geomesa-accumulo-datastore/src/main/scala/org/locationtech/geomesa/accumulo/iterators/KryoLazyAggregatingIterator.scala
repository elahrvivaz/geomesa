...
abstract class KryoLazyAggregatingIterator[T <: AnyRef { def isEmpty: Boolean; def clear(): Unit }]
  ...
  override def init(src: SortedKeyValueIterator[Key, Value],
    ...
    reusableSf = if (sft.getSchemaVersion < 9) {
      getId = (_) => reusableSf.getID
      new KryoFeatureSerializer(sft).getReusableFeature
    } else {
      ...
      new KryoFeatureSerializer(sft, SerializationOptions.withoutId).getReusableFeature
    }
