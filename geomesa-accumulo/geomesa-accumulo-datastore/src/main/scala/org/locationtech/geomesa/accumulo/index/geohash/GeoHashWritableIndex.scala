...
object GeoHashWritableIndex extends AccumuloWritableIndex with LazyLogging {
  ...
  override def configure(sft: SimpleFeatureType, tableName: String, ops: AccumuloDataStore): Unit = {
    ...
    ops.tableOps.setProperty(tableName, Property.TABLE_BLOOM_KEY_FUNCTOR.getKey, classOf[ColumnFamilyFunctor].getCanonicalName)
    ops.tableOps.setProperty(tableName, Property.TABLE_BLOOM_ENABLED.getKey, "true")
    ops.tableOps.setProperty(tableName, Property.TABLE_BLOCKCACHE_ENABLED.getKey, "true")
    ops.tableOps.setProperty(tableName, Property.TABLE_SPLIT_THRESHOLD.getKey, "128M")
  }
