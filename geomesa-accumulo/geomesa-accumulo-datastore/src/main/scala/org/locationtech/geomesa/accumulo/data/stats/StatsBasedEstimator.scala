...
class CountEstimator(sft: SimpleFeatureType, stats: GeoMesaStats) extends LazyLogging {
  ...
  private def estimateSpatioTemporalCount(filter: And): Option[Long] = {
    ...
    for {
      ...
      geometries =  FilterHelper.extractGeometries(filter, geomField)
  ...
  private def estimateSpatialCount(filter: Filter): Option[Long] = {
    ...
    val geometries = FilterHelper.extractGeometries(filter, sft.getGeomField)
    if (geometries.isEmpty) { None } else {
      stats.getStats[RangeHistogram[Geometry]](sft, Seq(sft.getGeomField)).headOption.map { histogram =>
        val (zLo, zHi) = {
        ...
        val ranges = Z2SFC.ranges(geometries.map(bounds), ZHistogramPrecision)
