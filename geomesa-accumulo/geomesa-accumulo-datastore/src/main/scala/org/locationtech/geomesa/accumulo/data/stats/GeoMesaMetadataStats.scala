...
class GeoMesaMetadataStats(ds: AccumuloDataStore, initialDelayMinutes: Int = 10) extends
  ...
  private def writeMinMax(stat: MinMax[_], sft: SimpleFeatureType, merge: Boolean = false): Unit = {
    ...
    if (!existing.contains(updated)) {
      val value = serialize(sft, updated)
  ...
  private def serialize(sft: SimpleFeatureType, stat: Stat): String = {
    val serializer = serializers.synchronized(serializers.getOrElseUpdate(sft.getTypeName, StatSerializer(sft)))
    Base64.encodeBase64String(serializer.serialize(stat))
  }

  private def deserialize(sft: SimpleFeatureType, stat: String): Stat = {
    val serializer = serializers.synchronized(serializers.getOrElseUpdate(sft.getTypeName, StatSerializer(sft)))
    serializer.deserialize(Base64.decodeBase64(stat))
  }
}
