...
import org.locationtech.geomesa.accumulo.index.z2.Z2IndexWritable
...
class Z2Iterator extends SortedKeyValueIterator[Key, Value] {
  ...
  override def init(source: SortedKeyValueIterator[Key, Value],
    ...
    val numBytes = if (isPoints) 8 else Z2IndexWritable.GEOM_Z_NUM_BYTES
...
object Z2Iterator {
  ...
  private def decodeNonPoints(x: Double, y: Double): (Int, Int) =
    Z2(Z2SFC.index(x, y).z & Z2IndexWritable.GEOM_Z_MASK).decode
}
