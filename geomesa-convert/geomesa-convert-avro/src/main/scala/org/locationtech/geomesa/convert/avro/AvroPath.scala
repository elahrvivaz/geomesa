...
object AvroPath extends BasicParser {
  ...
  private def convert(record: AnyRef): Option[AnyRef] = {
    record match {
      case x: Utf8                 => Some(x.toString)
      case x: ByteBuffer           => Option(convertBytes(x))
      case x: GenericFixed         => Option(x.bytes())
      case x: GenericEnumSymbol    => Option(x.toString)
      case x: GenericArray[AnyRef] => Option(x).map(convertList)
      case x                       => Option(x)
    }
  }
  ...
  private def convertList(list: java.util.List[AnyRef]): java.util.List[AnyRef] = {
    val result = new java.util.ArrayList[AnyRef](list.size())
    val iter = list.iterator()
    while (iter.hasNext) {
      result.add(convert(iter.next()).orNull)
    }
    result
  }
  ...
  case class CompositeExpr(se: Seq[AvroPath]) extends AvroPath {
    override def eval(r: AnyRef): Option[AnyRef] = r match {
      case gr: GenericRecord => se.foldLeft[Option[AnyRef]](Some(gr))((result, current) => result.flatMap(current.eval))
      case _ => None
    }
  }
