...
object OsmField {
  ...
  def requiresMetadata(attribute: OsmAttribute): Boolean =
    Seq(OsmAttribute.user, OsmAttribute.uid, OsmAttribute.version, OsmAttribute.changeset, OsmAttribute.timestamp).contains(attribute)
