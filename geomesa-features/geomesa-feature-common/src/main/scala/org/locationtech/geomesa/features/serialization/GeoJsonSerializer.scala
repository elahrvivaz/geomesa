...
class GeoJsonSerializer(sft: SimpleFeatureType) extends LazyLogging {
  ...
  private val geometryWriter = new GeometryWriter("geometry", sft.indexOf(sft.getGeometryDescriptor.getLocalName))

  private val attributeWriters = sft.getAttributeDescriptors.asScala.flatMap { d =>
    if (d == sft.getGeometryDescriptor) { Seq.empty } else {
      val name = d.getLocalName
      val i = sft.indexOf(name)
      val bindings = ObjectType.selectType(d)
      bindings.head match {
        case ObjectType.STRING if bindings.last == ObjectType.JSON => Seq(new JsonStringWriter(name, i))
        case ObjectType.STRING   => Seq(new StringWriter(name, i))
        case ObjectType.INT      => Seq(new NumberWriter(name, i))
        case ObjectType.LONG     => Seq(new NumberWriter(name, i))
        case ObjectType.FLOAT    => Seq(new NumberWriter(name, i))
        case ObjectType.DOUBLE   => Seq(new NumberWriter(name, i))
        case ObjectType.DATE     => Seq(new DateWriter(name, i))
        case ObjectType.GEOMETRY => Seq(new GeometryWriter(name, i))
        case ObjectType.UUID     => Seq(new UuidWriter(name, i))
        case ObjectType.BYTES    => Seq(new BytesWriter(name, i))
        case ObjectType.BOOLEAN  => Seq(new BooleanWriter(name, i))
        case ObjectType.LIST     => Seq(new ListWriter(name, i, bindings(1)))
        case ObjectType.MAP      => Seq(new MapWriter(name, i, bindings(1), bindings(2)))

        case b => logger.warn(s"Dropping attribute '$name' due to unexpected type binding: $b"); Seq.empty
      }
    }
  }
...
object GeoJsonSerializer {
  ...
  def writer(wrapped: Writer): JsonWriter = gson.newJsonWriter(wrapped)
