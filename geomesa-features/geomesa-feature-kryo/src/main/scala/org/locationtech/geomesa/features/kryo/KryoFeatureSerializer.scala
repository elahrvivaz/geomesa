...
class ProjectingKryoFeatureSerializer(original: SimpleFeatureType,
  ...
  private val cacheKey = CacheKeyGenerator.cacheKeyForSFT(projected)
...
trait KryoSimpleFeatureSerialization extends SimpleFeatureSerializer {
  ...
  private val cacheKey = CacheKeyGenerator.cacheKeyForSFT(serializeSft)
...
trait KryoSimpleFeatureDeserialization extends SimpleFeatureSerializer {
  ...
  private val cacheKey = CacheKeyGenerator.cacheKeyForSFT(deserializeSft)
...
class ProjectingKryoFeatureDeserializer(original: SimpleFeatureType,
  ...
  private def setup(): Unit = {
    val originalReaders = getReaders(CacheKeyGenerator.cacheKeyForSFT(original), original)
    var i = 0
