...
package org.locationtech.geomesa.features.avro.old.serialization
...
object AvroUserDataSerialization extends GenericMapSerialization[Encoder, Decoder] {
  ...
  override protected def writeBytes(
      out: Encoder, bytes: Array[Byte]): Unit = ???

  override protected def readBytes(in: Decoder): Array[Byte] = ???

  override protected def writeList(
      out: Encoder,
      list: util.List[AnyRef]): Unit = ???

  override protected def readList(in: Decoder): util.List[AnyRef] = ???
}
