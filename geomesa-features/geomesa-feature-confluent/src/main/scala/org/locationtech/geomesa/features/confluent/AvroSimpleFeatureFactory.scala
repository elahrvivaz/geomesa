...
class ConfluentSimpleFeatureFactory extends AbstractFeatureFactoryImpl {

  override def createSimpleFeature(attrs: Array[AnyRef],
                                   sft: SimpleFeatureType,
                                   id: String): SimpleFeature = {
    val f = new ConfluentSimpleFeature(new FeatureIdImpl(id), sft)
...
object ConfluentSimpleFeatureFactory {
  def init = {
    Hints.putSystemDefault(Hints.FEATURE_FACTORY, classOf[ConfluentSimpleFeatureFactory])
  }
  ...
  private val hints = new Hints(Hints.FEATURE_FACTORY, classOf[ConfluentSimpleFeatureFactory])
  private val featureFactory = CommonFactoryFinder.getFeatureFactory(hints)

  def buildConfluentFeature(sft: SimpleFeatureType, attrs: Seq[AnyRef], id: String) =
