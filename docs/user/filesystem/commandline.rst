...
Compact one or more filesystem partitions. This will merge multiple files fewer, larger files, which may
...
``--target-file-size``   Target size to compact files to
...
``--target-file-size``   Target size for data files
``--temp-path``          Path to a temp directory used for working files
