...
object CachedThreadPoolOps {
   ...
   */
  def execute[T](command: () => T): Unit = CachedThreadPool.execute(new Runnable() { override def run(): Unit = command() })
   ...
   * @return
   */
  def submit[T](command: () => T): Future[_] = CachedThreadPool.submit(new Runnable() { override def run(): Unit = command() })
