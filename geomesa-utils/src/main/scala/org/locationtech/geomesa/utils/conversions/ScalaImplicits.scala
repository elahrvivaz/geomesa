...
object ScalaImplicits {
  ...
  object TryWithWarning {
    def apply[T](r: => T)(logger: Logger, message: => String): Try[T] = {
      Try(r) match {
        case s: Success[T] => s
        case f: Failure[T] => logger.warn(message, f.exception); f
      }
    }
  }
  ...
  object TryWithError {
    def apply[T](r: => T)(logger: Logger, message: => String): Try[T] = {
      Try(r) match {
        case s: Success[T] => s
        case f: Failure[T] => logger.error(message, f.exception); f
      }
    }
  }
}
