...
class HistogramJob(args: Args) extends GeoMesaBaseJob(args) {
  ...
  // verify input params - inside a block so they don't get serialized
  {
    ...
    assert(groupBy.length < 8, "Can't group by more than 7 attributes")
  }
  ...
  val unique: TypedPipe[SimpleFeature] = if (uniqueBy.isEmpty) { features } else {
    features.distinctBy(uniqueByAttributes)(uniqueByOrdering)
  }

  unique.map(sf => s"${uniqueByAttributes(sf).productIterator.mkString("\t")}")
      .groupAll
      .sorted
      .values
...
object HistogramJob {
  ...
  val UNIQUE_BY   = "geomesa.hist.unique.attribute"
  ...
  def runJob(conf: Configuration,
    ...
    val args = Seq(FEATURE_IN  -> List(feature),
                   ...
                   GROUP_BY    -> groupBy).toMap ++ toInArgs(dsParams)
