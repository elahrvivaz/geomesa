...
case class ArrowAdapter(sft: SimpleFeatureType,
  ...
  // type returned from the kudu row
  private val returnSft = attributes match {
    case None => sft
    case Some(a) =>
      val sub = DataUtilities.createSubType(sft, a.toArray)
      sub.getUserData.putAll(sft.getUserData)
      sub
  }
  ...
  private val deserializer = schema.deserializer(returnSft)
  private val feature = new ScalaSimpleFeature(returnSft, "")
  private val transformFeature = transform.map { case (tdefs, tsft) =>
    val sf = TransformSimpleFeature(returnSft, tsft, tdefs)
  ...
  override def adapt(results: CloseableIterator[RowResult]): CloseableIterator[SimpleFeature] = {
    ...
    val features = results.flatMap { row =>
      ...
      if ((vis == null || VisibilityEvaluator.parse(vis).evaluate(auths)) && ecql.forall(_.evaluate(feature))) {
        ...
        Iterator.single(transformFeature.getOrElse(feature))
      } else {
...
object ArrowAdapter extends KuduResultAdapterSerialization[ArrowAdapter] {
  ...
  override def deserialize(bb: ByteBuffer): ArrowAdapter = {
    ...
    val dictionaryFields = (0 until bb.getInt).map(_ => bb.getString)
