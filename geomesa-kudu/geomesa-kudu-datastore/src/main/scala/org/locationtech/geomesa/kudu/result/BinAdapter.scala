...
case class BinAdapter(sft: SimpleFeatureType,
  ...
  private val encoder = {
    val g = geom.map(subType.indexOf)
    val d = dtg.map(subType.indexOf)
    BinaryOutputEncoder(sft, EncodingOptions(g, d, trackId.map(subType.indexOf), label.map(subType.indexOf)))
  }
