...
class FilterHelperTest extends Specification {
  ...
  def updateFilter(filter: Filter) = filter.accept(new SafeTopologicalFilterVisitorImpl(null), null).asInstanceOf[Filter]
  ...
  "FilterHelper" should {
    ...
    "fix out of bounds bbox" in {
      ...
      ECQL.toCQL(updated) mustEqual "BBOX(geom, -180.0,-90.0,180.0,90.0)"
    }
