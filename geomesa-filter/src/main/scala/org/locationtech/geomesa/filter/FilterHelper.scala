...
object FilterHelper extends LazyLogging {
  ...
  /**
    ...
    */
  def extractGeometries(filter: Filter, attribute: String): Seq[Geometry] =
    extractUnclippedGeometries(filter, attribute).map(_.intersection(WholeWorldPolygon))
  ...
  /**
    ...
    * @return geometry bounds from spatial filters
    */
  private def extractUnclippedGeometries(filter: Filter, attribute: String): Seq[Geometry] = filter match {
    case o: Or  => o.getChildren.flatMap(extractUnclippedGeometries(_, attribute))

    case a: And =>
      val all = a.getChildren.map(extractUnclippedGeometries(_, attribute)).filter(_.nonEmpty)
      if (all.isEmpty) { Seq.empty } else {
