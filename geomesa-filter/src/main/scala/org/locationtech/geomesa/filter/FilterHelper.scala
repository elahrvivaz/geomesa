...
object FilterHelper {
  ...
  def visitBBOX(op: BBOX, sft: SimpleFeatureType): Filter = {
    ...
    geom.getUserData match {
      ...
      case _ =>
        ...
        safeGeometry.setUserData("safe")
        updateToIDLSafeFilter(op, safeGeometry, attribute, prop.flipped)
    }
  }
  ...
  def visitDwithin(op: DWithin, sft: SimpleFeatureType): Filter = {
    ...
    geom.getUserData match {
      ...
      case _ =>
        ...
        updateToIDLSafeFilter(op, safeGeometry, attribute, prop.flipped, distanceDegrees)
    }
  }

  private def updateToIDLSafeFilter(op: BinarySpatialOperator,
                                    geom: Geometry,
                                    attribute: String,
                                    flipped: Boolean,
                                    args: Any = null): Filter = {
    geom match {
      case g: GeometryCollection => ff.or(getGeometryListOf(g).map(recreateFilter(op, attribute, _, flipped, args)))
      case _ => recreateFilter(op, attribute, geom, flipped, args)
    }
  }
  ...
  private def recreateFilter(op: BinarySpatialOperator,
    ...
    op match {
      ...
      // note: The ECQL spec doesn't allow for us to put the measurement in "degrees", but that's how this filter will be used.
      case op: DWithin    => ff.dwithin(e1, e2, args.asInstanceOf[Double], "meters")
      case op: BBOX =>
        val envelope = geom.getEnvelopeInternal
        ff.bbox(ff.property(property), envelope.getMinX, envelope.getMinY, envelope.getMaxX, envelope.getMaxY, op.getSRS)
    }
  }
